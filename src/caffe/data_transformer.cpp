...
void distort_image(cv::Mat &im, Dtype hue, Dtype sat, Dtype val)
{
    ...
    cvtColor(im_scale, im_hsv, cv::COLOR_BGR2HSV);
    ...
    cvtColor(im_hsv, im_scale, cv::COLOR_HSV2BGR);
...
{
    img_crop =  cv::Mat(h, w, img_org.type(), cv::Scalar::all(128));
...
void pad_image(cv::Mat &img_crop, vector<BoxLabel>& box_labels)
{
    ...
    {
      int sy = (ow - oh) / 2;
      cv::Mat img_pad =  cv::Mat(ow, ow, img_crop.type(), cv::Scalar::all(128));
      cv::Mat roi_pad = img_pad(cv::Rect(0, sy, ow, oh));
      img_crop.copyTo(roi_pad);
      for(int i =0 ; i< box_labels.size(); i++)
      {
    ...
    {
      int sx = (oh - ow) / 2;
      cv::Mat img_pad =  cv::Mat(oh, oh, img_crop.type(), cv::Scalar::all(128));
      cv::Mat roi_pad = img_pad(cv::Rect(sx, 0, ow, oh));
      img_crop.copyTo(roi_pad);
      for(int i =0 ; i< box_labels.size(); i++)
      {
...
void draw_box(cv::Mat img, vector<BoxLabel> & box_vec)
{
  ...
  for(int i=0; i<box_vec.size(); i++)
  {
    ...
    cv::rectangle(img, p1, p2, cv::Scalar(255, 0, 0), 1);
  }
}
  ...
  for(int i=0; i<kps_vec.size(); i++)
  {
    for(int j=0; j<17;j++)
    {
      if(kps_vec[i].kps_x_[j] !=0 && kps_vec[i].kps_y_[j]!=0)
      {
        ...
        cv::circle(img, p1, 2, cv::Scalar(255, 255, 255), 1);
