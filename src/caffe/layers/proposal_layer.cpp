...
static void _whctrs(vector <float> anchor, vector<float> &ctrs) {
...
static void _mkanchors(vector<float> ctrs, vector<float> &anchors) {
...
template <typename Dtype>
static void _ratio_enum(vector<float> anchor, const Dtype ratios[], const int num_ratios, vector<float> &anchors_ratio) {
	vector<float> ctrs;
	_whctrs(anchor, ctrs);
	float size = ctrs[0] * ctrs[1];
	for (int i = 0; i < num_ratios; i++)
	{
		float ratio = size / ratios[i];
		int ws = int(round(sqrt(ratio)));
		int hs = int(round(ws * ratios[i]));
...
template <typename Dtype>
static void _scale_enum(vector<float> anchors_ratio, const Dtype scales[], const int num_scales, vector<float> &anchor_boxes) {
	...
	for (int i = 0; i < anchors_ratio_num; i++)
	{
		...
		{
			float ws = ctrs[0] * scales[j];
			float hs = ctrs[1] * scales[j];
			vector<float> ctrs_in;
			ctrs_in.push_back(ws);
			ctrs_in.push_back(hs);
			ctrs_in.push_back(ctrs[2]);
	...
	Dtype anchors[])
{
	...
	_ratio_enum(base_anchor, ratios, num_ratios, anchors_ratio);
	vector<float> anchor_boxes;
	_scale_enum(anchors_ratio, scales, num_scales, anchor_boxes);
	for (int j = 0; j < anchor_boxes.size(); j++)
	{
		anchors[j] = anchor_boxes[j];
	}
}
                                      ...
                                      const vector<Blob<Dtype> *> &top)
{
  ...
  // Dtype *p_anch = anchors_.mutable_cpu_data();
  // for (int n = 0; n < anchors_shape[0]; ++n)
  //{
	 // std::cout <<""<<p_anch[n*4+0]<<", "<<p_anch[n*4+1]<<", "<<p_anch[n*4+2]<<", "<<p_anch[n*4+3]<<std::endl;
  //}
                                       ...
                                       const vector<Blob<Dtype> *> &top)
{
  ...
  for (int n = 0; n < bottom[0]->shape(0); ++n)
  {
    ...
    int valid_proposal_cnt=enumerate_proposals_cpu(
        ...
        feat_stride_);
    proposals_shape[0] = valid_proposal_cnt;
    proposals_.Reshape(proposals_shape);

    //sort_box(proposals_.mutable_cpu_data(), 0, num_proposals - 1, pre_nms_topn_);
    sort_box(proposals_.mutable_cpu_data(), 0, valid_proposal_cnt - 1, pre_nms_topn_);
