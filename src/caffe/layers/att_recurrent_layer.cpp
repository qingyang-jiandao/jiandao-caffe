...
namespace caffe {
	...
	void AttRecurrentLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		CHECK_GE(bottom[0]->num_axes(), 2)
			<< "bottom[0] must have at least 2 axes -- (#timesteps, #streams, ...)";

		N_ = bottom[0]->shape(0);
		SEQ_NUM_ = bottom[0]->shape(1);
		T_ = SEQ_NUM_;
		//T_ = 16;
		...
		input_layer_param->set_name("unrollednet_input_data");
		...
		y_input_blob_ = CHECK_NOTNULL(unrolled_net_->blob_by_name("y").get());

		// Setup pointers to paired recurrent inputs/outputs.
		recur_input_blobs_.resize(recur_input_names.size());
		recur_output_blobs_.resize(recur_output_names.size());
		...
		// We should have 2 inputs (x and y), plus a number of recurrent inputs,
		...
		for (int i = 0; i < unrolled_net_->layers().size(); ++i) {
			for (int j = 0; j < unrolled_net_->layers()[i]->blobs().size(); ++j) {
				//LOG(INFO) << "layer id:" << i << " layer name:" << layer_names[i] 
				//	<< " propagate_down:"<< unrolled_net_->layers()[i]->param_propagate_down(j);
		...
		for (int i = 0; i < recur_input_blobs_.size(); ++i) {
			...
			caffe_set(recur_input_blobs_[i]->count(), Dtype(0),
				recur_input_blobs_[i]->mutable_cpu_data());
		}

		cont_input_blob_ = unrolled_net_->blob_by_name("cont_0").get();
		if (cont_input_blob_) {
			Dtype* timestep_cout_0_data = cont_input_blob_->mutable_cpu_data();
			caffe_set(cont_input_blob_->count(), Dtype(1),
				timestep_cout_0_data);
		}
	...
	template <typename Dtype>
	void AttRecurrentLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		N_ = bottom[0]->shape(0);
		SEQ_NUM_ = bottom[0]->shape(1);
		T_ = SEQ_NUM_;
		...
		vector<int> y_shape = bottom[1]->shape();
		y_input_blob_->Reshape(y_shape);
		...
		y_input_blob_->ShareData(*bottom[1]);
	...
	template <typename Dtype>
	void AttRecurrentLayer<Dtype>::Reset() {
	...
	template <typename Dtype>
	void AttRecurrentLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top) {
		...
		LOG(INFO) << " AttRecurrentLayer Forward_cpu";
		// initializaion h_0 c_0
		if (1) {
			//for (int i = 0; i < recur_output_blobs_.size(); ++i) {
			//	const int count = recur_input_blobs_[i]->count();
	...
	template <typename Dtype>
	void AttRecurrentLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
		CHECK(!propagate_down[1]) << "Cannot backpropagate to sequence indicators.";

		//LOG(INFO) << " ******************************AttRecurrentLayer Backward_cpu";
		//LOG(INFO) << " ******************************AttRecurrentLayer Backward_cpu";
...
#ifdef CPU_ONLY
	STUB_GPU_FORWARD(AttRecurrentLayer, Forward);
#endif

	INSTANTIATE_CLASS(AttRecurrentLayer);
