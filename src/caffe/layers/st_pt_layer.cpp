...
void SpatialTransformerPTLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void SpatialTransformerPTLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
Dtype SpatialTransformerPTLayer<Dtype>::transform_forward_cpu(const Dtype* pic, Dtype px, Dtype py) {
...
template <typename Dtype>
void SpatialTransformerPTLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
	...
	for(int i = 0; i < N; ++i) {
		...
		Dtype* coordinates = input_grid_data + (output_H_ * output_W_ * 3) * i;

		caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, output_H_ * output_W_, 3, 3, (Dtype)1.,
		      output_grid_data, theta + 9 * i, (Dtype)0., coordinates);
...
template <typename Dtype>
void SpatialTransformerPTLayer<Dtype>::transform_backward_cpu(Dtype dV, const Dtype* U, const Dtype px,
...
template <typename Dtype>
void SpatialTransformerPTLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(SpatialTransformerPTLayer);
#endif

INSTANTIATE_CLASS(SpatialTransformerPTLayer);
REGISTER_LAYER_CLASS(SpatialTransformerPT);
