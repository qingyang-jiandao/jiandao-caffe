...
void InterpLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  } else if (interp_param.has_scale_factor_h() && interp_param.has_scale_factor_w()) {
    const float scale_factor_h  = interp_param.scale_factor_h();
    const float scale_factor_w  = interp_param.scale_factor_w();
	height_out_ = (height_in_eff_ - 1) * scale_factor_h + 1;
    width_out_ = (width_in_eff_ - 1) * scale_factor_w + 1;
  } else {
	if(bottom.size()>1){
