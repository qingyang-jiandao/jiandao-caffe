...
void Yolov3DetectionOutputLayer<Dtype>::Forward_cpu(
    ...
    if(predicts.size() > 0) {
		...
		if (predicts.size() > 300)
		{
			typename std::vector<PredictionResult<Dtype>>::const_iterator first1 = predicts.begin();
			typename std::vector<PredictionResult<Dtype>>::const_iterator last1  = predicts.begin() + 300;
			std::vector<PredictionResult<Dtype>> pre_predicts(first1, last1);
			ApplyNms(pre_predicts, idxes, nms_threshold_);
			predicts.clear();
			predicts = pre_predicts;
		}
		else
		{
		...
		if (idxes.size() > 64) 
		{
			typename std::vector<int>::const_iterator first1 = idxes.begin();
			typename std::vector<int>::const_iterator last1  = idxes.begin() + 64;
			std::vector<int> post_idxes(first1, last1);
			idxes.clear();
			idxes = post_idxes;
			num_kept = idxes.size();
		}
		...
		//std::cout <<"------------------------------" << idxes.size() << nms_threshold_ <<std::endl;
		//ApplyNms(predicts, idxes, nms_threshold_);
		//std::cout <<"------------------------------" <<std::endl;
		//num_kept = idxes.size();
		//LOG(INFO) << num_kept;
