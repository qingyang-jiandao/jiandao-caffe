...
void SpatialTransformerPTOPLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {

	string prefix = "\t\tSpatial Transformer PT Layer:: LayerSetUp: \t";
	...
	if (this->layer_param_.st_param().has_theta_3_1()) {
	...
	if (this->layer_param_.st_param().has_theta_3_2()) {
	...
	if (this->layer_param_.st_param().has_theta_3_3()) {
		is_pre_defined_theta[8] = true;
		++pre_defined_count;
		pre_defined_theta[8] = this->layer_param_.st_param().theta_3_3();
		std::cout << prefix << "Getting pre-defined theta[3][3] = " << pre_defined_theta[8] << std::endl;
	}

	is_pre_defined_theta[9] = false;
	if (this->layer_param_.st_param().has_theta_o_x()) {
		is_pre_defined_theta[9] = true;
		++pre_defined_count;
		pre_defined_theta[9] = this->layer_param_.st_param().theta_o_x();
		std::cout << prefix << "Getting pre-defined theta_o_x = " << pre_defined_theta[9] << std::endl;
	}

	is_pre_defined_theta[10] = false;
	if (this->layer_param_.st_param().has_theta_o_y()) {
		is_pre_defined_theta[10] = true;
		++pre_defined_count;
		pre_defined_theta[10] = this->layer_param_.st_param().theta_o_y();
		std::cout << prefix << "Getting pre-defined theta_o_y = " << pre_defined_theta[10] << std::endl;
	}

	// check the validation for the parameter theta
	CHECK(bottom[1]->count(1) + pre_defined_count == 11) << "The dimension of theta is not 11!"
	...
	//int thete_count = bottom[1]->shape(1);
	//std::cout << "thete_count " << thete_count << std::endl;

	std::cout<<prefix<<"Initialization finished."<<std::endl;
}

template <typename Dtype>
void SpatialTransformerPTOPLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {

	string prefix = "\t\tSpatial Transformer PT Layer:: Reshape: \t";
	...
	output_H_ = H;
	output_W_ = W;
	if (init_output_H_ != 0 && init_output_W_ != 0) {
		output_H_ = init_output_H_;
		output_W_ = init_output_W_;
	}
	vector<int> shape_output(2);
	shape_output[0] = output_H_ * output_W_; shape_output[1] = 3;
	output_grid.Reshape(shape_output);
	...
	inner_grid.Reshape(shape_input);
	...
	dTheta_tmp_shape[2] = output_H_ * output_W_ * C;
	dTheta_tmp.Reshape(dTheta_tmp_shape);

	dTheta_tmp_shape[0] = N;
	dTheta_tmp_shape[1] = 2;
	dTheta_tmp_shape[2] = output_H_ * output_W_ * C;
	dTheta_tmp_point_xy.Reshape(dTheta_tmp_shape);
	...
	full_theta_shape[0] = N;
	full_theta_shape[1] = 2;
	full_theta_point_xy.Reshape(full_theta_shape);

	if(global_debug) std::cout<<prefix<<"Finished."<<std::endl;
}

template <typename Dtype>
Dtype SpatialTransformerPTOPLayer<Dtype>::transform_forward_cpu(const Dtype* pic, Dtype px, Dtype py) {

	bool debug = false;

	string prefix = "\t\tSpatial Transformer PT Layer:: transform_forward_cpu: \t";
...
template <typename Dtype>
void SpatialTransformerPTOPLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {

	string prefix = "\t\tSpatial Transformer PT Layer:: Forward_cpu: \t";
...
template <typename Dtype>
void SpatialTransformerPTOPLayer<Dtype>::transform_backward_cpu(Dtype dV, const Dtype* U, const Dtype px,
		const Dtype py, Dtype* dU, Dtype& dpx, Dtype& dpy) {

	bool debug = false;

	string prefix = "\t\tSpatial Transformer PT Layer:: transform_backward_cpu: \t";
...
template <typename Dtype>
void SpatialTransformerPTOPLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down,
    const vector<Blob<Dtype>*>& bottom) {

		string prefix = "\t\tSpatial Transformer PT Layer:: Backward_cpu: \t";
...
#ifdef CPU_ONLY
STUB_GPU(SpatialTransformerPTOPLayer);
#endif

INSTANTIATE_CLASS(SpatialTransformerPTOPLayer);
REGISTER_LAYER_CLASS(SpatialTransformerPTOP);
