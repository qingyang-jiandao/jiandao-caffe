...
int main(int argc, char** argv) {
  ...
  const string& names_file = argv[4];
	  ...
	  for (size_t k = 0; k < fn.size(); ++k)
	  {
		  ...
		  for (int i = 0; i < detections.size(); ++i) {
			  ...
			  if (score >= confidence_threshold) {
				  ...
				  pt2.x = (img.cols*d[5]);
				  pt2.y = (img.rows*d[6]);
				  //out << pt1.x << " " << pt1.y << " " << pt2.x << " " << pt2.y << std::endl;
				  //out << img.cols << " " << img.rows  << std::endl;
				  cv::rectangle(img, pt1, pt2, cvScalar(0, 255, 0), 1, 8, 0);
				  //out << obj_names[static_cast<int>(d[1])-1]  << std::endl;
				  char label[100];
				  //sprintf(label, "%s,%f", CLASSES[static_cast<int>(d[1])], score);
				  sprintf(label, "%s,%f", obj_names[static_cast<int>(d[1])-1].c_str(), score);
				  int baseline;
